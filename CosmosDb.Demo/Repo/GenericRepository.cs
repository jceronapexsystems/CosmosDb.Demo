...
{
    public abstract class GenericRepository<T>(IUnitOfWorkFactory unitOfWorkFactory)
    : IGenericRepository<T> where T : BaseEntity, new()
    {
        private readonly IUnitOfWorkFactory _unitOfWorkFactory = unitOfWorkFactory;

        protected abstract ConsistencyLevel _consistencyLevel { get; }
        ...
        public async Task<ItemResponse<T>> Create(T entity)
        {
            ...
            using var _unitOfWork = _unitOfWorkFactory.GetUnitOfWork<T>(_consistencyLevel, ConnectionString, entity.Region);
